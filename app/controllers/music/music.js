...
var CommentMusic = require('../../models/movie/movie_comment');   //音乐评论模型
var MusicCategory = require('../../models/music/music_category'); //音乐分类模型
var _ = require('underscore');   //该模块用来对变化字段进行更新
...
exports.savePoster = function(req, res, next){
	...
	var originalFilename = imageData.originalFilename;//原始名字

	if(originalFilename){
		fs.readFile(filePath,function(err,data){
			var timestamp = Date.now();  //获取时间
			var type = imageData.type.split('/')[1]; //获取图片类型 如jpg png
			var image = timestamp + '.' + type;   //上传海报新名字
...
exports.save = function(req,res){
	...
	if(id){
		...
		var musicCategoryName = musicObj.musicCategoryName;
