$(function(){
	...
	*/
					...
					for(var i = 0; i < dataCars.length; i ++) {
						...
						if(dataCars[i].musics.length < oALen) {
							...
							if(dataCars[i].musics.length > 2){
								$('#hotProgrammes .thumbnail:eq('+ i +') .inner').append('<span>...</span>');
							}
						}
						// 歌单中歌曲数量相同，则只替换相应内容，节点数量不变
						for(var j = 0; j < dataCars[i].musics.length; j++) {
							$(oThumbnail[i]).find('a:eq('+ j +')').attr('href','/music/' + dataCars[i].musics[j]._id).attr('title',dataCars[i].musics[j].title);
	...
	/*
		本周单曲榜区点击切换榜单内容事件
	 */
	$('#hotArtistSongs').on('click','li',function() {
		// 只有点击不同按钮才触发Ajax事件，避免对同一个按钮重复点击触发请求
		if($(this).is('.on')){
			return;
		}else{
